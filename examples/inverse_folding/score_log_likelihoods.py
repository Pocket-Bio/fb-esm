...
def score_singlechain_backbone(model, alphabet, args):
    ...
    coords, native_seq = fair_esm.inverse_folding.util.load_coords(args.pdbfile, args.chain)
    print('Native sequence loaded from structure file:')
    print(native_seq)
    print('\n')

    ll, _ = fair_esm.inverse_folding.util.score_sequence(
...
def score_multichain_backbone(model, alphabet, args):
    if torch.cuda.is_available() and not args.nogpu:
        model = model.cuda()
        print("Transferred model to GPU")
    structure = fair_esm.inverse_folding.util.load_structure(args.pdbfile)
    coords, native_seqs = fair_esm.inverse_folding.multichain_util.extract_coords_from_complex(structure)
    ...
    ll, _ = fair_esm.inverse_folding.multichain_util.score_sequence_in_complex(
...
def main():
    ...
    model, alphabet = fair_esm.pretrained.esm_if1_gvp4_t16_142M_UR50()
    model = model.eval()
